...
; 
OUTCH:   .EQU $0FD7   ;output a character routine
INCH:    .EQU $0FDD   ;input a character routine 
PUTCH:   .EQU $3FAA   ;outch routine address
GETCH:   .EQU $3FAC   ;inch routine address
RST38:   .EQU $3FFA  ;interrupt vector
WARM:    .EQU $0FD0   ;serial monitor warm entry point
ENTRY:   .EQU $0F54   ;serial monitor post serial vectors
MSGBUF:  .EQU $3E00   ;string handling area
...
COMMANDPORT: .EQU $C3            ;write commands
DATAPORT:    .EQU $C2            ;read and write Top of Stack    

BUSY:      .EQU  $80             ;currently executing a command
APU_SIGN:  .EQU  $40             ;the value on the top of the stack is negative
ZERO:      .EQU  $20             ;the value on the top of the stack is zero
ERRORCODE: .EQU  $1E             ;indicated the validity of the previous operation
CARRY:     .EQU  $01             ;the previous operation resulted in a carry/borrow


;some fixed point 16 bit examples
SADD   .equ    $6c
FADD   .equ    $90
FSUB   .equ    $11
FMUL   .equ    $12
...
main1:  
        ld   hl,operator             
        call sndmsg                  ;what would you like to do?
        ld   a,'?'
        call OUTCH      
main2:
        call    INCH
        ...
        jp      Z,convert
        cp      'a'                 ;toggle the 9511A APU flag
        jp      nz,main2
        call    OUTCH  ;echo
        call    CRLF 
        call    printsz
...
;enter addend
;sum = addend + addend
fnadd:  call    OUTCH  ;echo
        call    CRLF 

        ld      hl,szAddition
        call    sndmsg          ;ask for addends
...
;
        ...
        or     a
        jr     z,fnadd1   
; use the apu to perform the addition 

        call    printsz
        .db     "APU FP Addition",CR,LF,0 
        call  apubusy
         ld   h,SCRPG
;         ld   l,OP1        ;put the 32 bit OP1 on the stack
        call  push32OP1
        ld    h,SCRPG
;         ld   l,OP2
        call  push32OP2       ;pu the 32 bit OP2 on the stack
;
; issue FADD command $10
;     
         ld   a,FADD 
         out  (COMMANDPORT),a 
         call  apubusy
         ld   h,SCRPG
         ld   l,OP1        ;put the 32 bit result in OP1
         call  pop32
        jp     fnadd2
...
apumsg:     .db  "AM9511A APU Test",CR,LF,$00
apuchkbsy:  .db  "Check Busy",CR,LF,$00
apudata:    .db  "Load data",CR,LF,$00  
apucommand: .db  "Issue command",CR,LF,$00
...
signon:     .db  "Lawrence Livermore Laboratory Floating Point Package",CR,LF,$00
operator:   .db  "Operation (+ - / x s(qrt) c(onvert)) ",CR,LF,$00

HELLO:          .db    CR,LF,"LLL Float ",0
APU_HELLO:      .db    CR,LF,"Am9511A Float ",0
PYTHAGORAS:     .db    CR,LF,"SQRT[ a^2 + b^2 ] ",0
szAddition:     .db    "Enter :Addends",CR,LF,0
szSum           .db       "Sum = ",0
szSubtraction:  .db    "Enter :Minuend :Subtrahend",CR,LF,0
szDifference:   .db    "Difference = ",0
szDivision:     .db      "Enter :Dividend :Divisor",CR,LF,0
szQuotient:     .db      "Quotient = ",0
szMultiplication   .db  "Enter :Factors",CR,LF,0
szProduct       .db  "Product = ",0
szConvert       .db  "Convert",0
szScientific:   .db  "Scientific Notation = ",0
szSquare        .db  "Enter :Square",CR,LF,0
szRoot          .db  "Square Root = ",0


INPUT_WD_PROMPT:    .db    CR,LF,"WD  Input  0x",0
INPUT_DWD_PROMPT:   .db    CR,LF,"DWD Input  0x",0
OUTPUT_WD_PROMPT:   .db    CR,LF,"WD  Output 0x",0
OUTPUT_DWD_PROMPT:  .db    CR,LF,"DWD Output 0x",0
